...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const paths = [
    path.join(process.cwd(), ".jdy_writer", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"),
      ".jdy_writer", "EXTEND.md"
    ),
    path.join(os.homedir(), ".jdy_writer", "EXTEND.md"),
  ];
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or .jdy_writer/.env file." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const cwdEnvPath = path.join(process.cwd(), ".jdy_writer", ".env");
  const homeEnvPath = path.join(os.homedir(), ".jdy_writer", ".env");
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<cwd>/.jdy_writer/.env (${prefixedKeyLabel})`, cwdEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
      buildCredentialSource(`~/.jdy_writer/.env (${prefixedKeyLabel})`, homeEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("<cwd>/.jdy_writer/.env", cwdEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("~/.jdy_writer/.env", homeEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
